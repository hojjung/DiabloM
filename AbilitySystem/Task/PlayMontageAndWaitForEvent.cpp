...
void UPlayMontageAndWaitForEvent::Activate()
{
	...
	UDiabloAbilitySystemComp* diaASC = GetTargetASC();
	...
	if (diaASC)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = diaASC->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UPlayMontageAndWaitForEvent::OnGameplayEvent));

			if (diaASC->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
{// Note: Clearing montage end delegate isn't necessary since its not a multicast and will be cleared when the next montage plays.
	...
	if (diaASC)
	{
		diaASC->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
...
UDiabloAbilitySystemComp * UPlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UDiabloAbilitySystemComp>(AbilitySystemComponent);
}
