...
{
	UNormalDungeonManager* Manager =  Cast<UNormalDungeonManager>( UDiabloGameInstance::Get->m_DungeonManager->GetCurrentSpawnManager());
	
	m_BarBossHp->SetProgressValue(per);
	
	if(Manager->GetBossMob())
	{
		if(Manager->GetBossMob()->m_fCurrentHP.IsLessThanZero())
		{
			UpdateBossText(0);
		}
		else
		{
			UpdateBossText(Manager->GetBossMob()->m_fCurrentHP);
		}
	}
}
...
{
	if(UDiabloGameInstance::Get->m_DungeonManager->GetCurrentSpawnManager()->IsBattleStarted() || m_fBossCooldownTimeCounter>0)
	{
		return;
	}
