...
{
	if (!m_PlayerDiaComp->IsTalentUnlock(m_CurrentTalent))
	{
...
{
	if (!m_CurrentTalent->IsLevelupable())
	{
...
{
	if (m_CurrentTalent && m_PlayerDiaComp->LevelupTalent(m_CurrentTalent))
	{
		SetTalentSpecData(m_CurrentTalent);
	}
}
